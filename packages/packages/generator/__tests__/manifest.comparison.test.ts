    ...
    availableConfigs.forEach((configName) => {
      describe(`Config: ${configName}`, () => {
        ...
        beforeAll(() => {
          ...
          comparison = comparator.compareManifests(
            generatedManifests,
            expectedManifests,
            options
          );
        ...
        test('should have reasonable resource counts', () => {
          ...
          if (chainTypes.hasEthereumChains) {
            const services = generatedManifests.filter(
              (r) => r.kind === 'Service'
            );
            const statefulSets = generatedManifests.filter(
          ...
          if (chainTypes.hasCosmosChains) {
            const configMaps = generatedManifests.filter(
              (r) => r.kind === 'ConfigMap'
            );
  ...
  describe('Cross-Config Analysis', () => {
    test('should produce consistent resource patterns across configs', () => {
      ...
      const results: Record<
        string,
        { generated: number; expected: number; chainTypes: ChainTypeConfig }
      > = {};

      // Analyze each config
      for (const configName of availableConfigs.slice(0, 5)) {
      ...
      // Analyze patterns
      const cosmosConfigs = Object.entries(results).filter(
        ([_, data]) => data.chainTypes.hasCosmosChains
      );
      const ethConfigs = Object.entries(results).filter(
        ([_, data]) => data.chainTypes.hasEthereumChains
      );

      if (cosmosConfigs.length > 1) {
        // Cosmos configs should have similar patterns
        const avgGenerated =
          cosmosConfigs.reduce((sum, [_, data]) => sum + data.generated, 0) /
          cosmosConfigs.length;
      ...
      if (ethConfigs.length > 1) {
        ...
        const avgGenerated =
          ethConfigs.reduce((sum, [_, data]) => sum + data.generated, 0) /
          ethConfigs.length;
      ...
      Object.entries(results).forEach(([configName, data]) => {
        console.log(
          `  ${configName}: Generated(${data.generated}) Expected(${data.expected}) Cosmos(${data.chainTypes.hasCosmosChains}) Eth(${data.chainTypes.hasEthereumChains})`
        );
      });
    });
  });
