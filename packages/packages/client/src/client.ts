...
const defaultVersion: string = 'v1.8.0';
...
export class StarshipClient implements StarshipClientI {
  ...
  public deploy(options: string[] = []): void {
    ...
    this.config.chains.forEach((chain: Chain, chainIndex: number) => {
      if (chain.scripts) {
  ...
  public startPortForward(): void {
    ...
    this.config.chains?.forEach((chain: Chain) => {
      const chainPodPorts =
    ...
    this.config.relayers?.forEach((relayer: Relayer) => {
    ...
    this.config.frontends?.forEach((frontend: Frontend) => {
      if (frontend.ports) {
